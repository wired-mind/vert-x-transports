...
public class VertxSender extends AbstractTransportSender {
    ...
    @Override
    public void sendMessage(final MessageContext messageContext, final String targetEPR, OutTransportInfo outTransportInfo) throws AxisFault {
        if (messageContext.getProperty(HttpAxis2ServerHandler.RESPONSE) != null) {
            ...
            //We tried to keep this on the main vertx thread, but that's not really possible now without forking axis2. We'll just switch threads to
            //make sure the actual send is from a vertx thread
            VertxLocator.vertx.runOnLoop(new SendHandler(messageContext, url));
        }
    }
    ...
    private class SendHandler implements Handler<Void> {
        private final MessageContext messageContext;
        private final URL url;

        public SendHandler(MessageContext messageContext, URL url) {
        ...
        public void handle(Void event) {
            ...
            HttpClient httpClient = (HttpClient) messageContext.getProperty(VertxConstants.HTTP_CLIENT);
